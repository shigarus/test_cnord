...
3) Т.к. слушателя интересует состояние и время последнего сообщение - считаем источника подключившимся
   только после первого корректного сообщения, т.к. по некорректному не понять статус.
4) Считать источника зарегестрировавшимся можно только после уведомления
   всех существующих слушателей о подключении.
4) Слушатель может подключиться во время регистрации источника. В данном случае он так же должен быть
   уведомлен о подключении этого источника.
4) Ничего не сказано о необходимости реализовывать гарантированную доставку - для упрощения разработки
   предполагаем, что гарантированная доставка не нужна, однако нужно написать систему так,
   чтобы встраивание гарантированной доставки не вызывало боль.
5) Нужно хранить состояние подключенных источников не в памяти приложения.
6) Ничего не сказано о дерегистрации источников и возможности вмешаться в протокол связи с ними - храним
   источники вечно.
